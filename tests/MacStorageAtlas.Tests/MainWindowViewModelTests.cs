...
public class MainWindowViewModelTests
{
    ...
    public void TreemapSelectionExposesSelectedItemDetails()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(viewModel.SelectedTreemapItemFormattedSize, Is.EqualTo("1.5 KB"));
            Assert.That(viewModel.HasSelectedTreemapItem, Is.True);
        });
    }
    ...
    public void ClearingTreemapSelectionClearsSelectedItemDetails()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(viewModel.SelectedTreemapItemFormattedSize, Is.Empty);
            Assert.That(viewModel.HasSelectedTreemapItem, Is.False);
        });
    }
    ...
    [Test]
    public async Task MoveToTrashCommandRemovesConfirmedItemFromScanResults()
    {
        var root = new DiskItem("root", "/scan/root", isDirectory: true);
        var item = new DiskItem("file.dat", "/scan/root/file.dat", isDirectory: false)
        {
