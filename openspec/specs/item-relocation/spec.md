...
Define how MacStorageAtlas moves or copies explicitly selected cleanup basket
items to a user-chosen destination, validates the destination before any
filesystem change, blocks protected, stale, colliding, or oversized relocations,
keeps a failed transfer from removing the source, reports itemized cancellable
outcomes, and preserves scan-result consistency and privacy boundaries.
