...
text, and shared-storage status; how filtered results are presented across the
result views; how match totals and exclusions are reported; and how filter
presets are defined, applied, and persisted, without rescanning or changing the
scan's measurement basis.
