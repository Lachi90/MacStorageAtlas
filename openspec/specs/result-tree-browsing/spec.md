...
Define how the folder tree presents a completed scan result as the user searches
and expands it, including which items are matched, that the displayed tree
reflects the most recent search text, and that browsing a large result stays
responsive without rescanning or changing the scan's measurement basis.
